...
│  ┌───────────────────────────────────────────────────┐   │
│  │             Discovery Engine                       │   │
│  │  Конфиг → Профили модулей → Поконтрольный фоллбэк │   │
│  └───────────────────────┬───────────────────────────┘   │
│                          │                               │
│                          ▼                               │
│  ┌───────────────────────────────────────────────────┐   │
...
2. **Discovery Engine** — определяет, как контролы превращаются в канонические устройства: по ручному конфигу, профилям модулей или поконтрольному фоллбэку.
3. **Каноническая модель** — собирает контролы в составные устройства, валидирует их по описаниям типов.
4. **Адаптеры** — получают канонические устройства и транслируют их в формат целевой платформы.
...
Объект `discovery` управляет автоматическим обнаружением устройств. Все поля опциональны; если объект отсутствует, используются значения по умолчанию.
...
|------|-----|:---:|----------|
| `enabled` | boolean | `true` | Включить автоматическое обнаружение |
| `exclude` | string[] | `[]` | Контролы, исключённые из обнаружения |
| `exclude_devices` | string[] | `[]` | MQTT-устройства, исключённые целиком |

Discovery Engine работает в двух режимах: автоматический (runtime) и генерация конфига (`--scan`). Подробная спецификация: [docs/discovery-engine.md](docs/discovery-engine.md).

### Полный пример конфига

Пример включает стандартные типы (простые и составные), кастомный тип (`tv`) и настройки обнаружения. Полная версия — `examples/wb-ext-bridge.conf`.
...
Discovery Engine автоматически обнаруживает устройства Wiren Board и создаёт канонические устройства. Три уровня приоритета:

1. **Ручной конфиг** (`devices[]`) — всегда выигрывает
2. **Профили модулей** — известные WB-модули (MDM3 → 3 диммера, MR6C → 6 реле, MSW-v3 → мультисенсор)
3. **Поконтрольный фоллбэк** — таблица соответствий по типу контрола (switch → switch, temperature → temperature_sensor)

Профили модулей позволяют создавать **составные устройства** автоматически. Например, WB-MDM3 превращается в 3 диммера (реле + канал яркости), а не в 6 отдельных сущностей.

Подробная спецификация: [docs/discovery-engine.md](docs/discovery-engine.md).
...
├── profiles/                          # Профили модулей WB
│   ├── wb-mdm3.yaml                   # 3-канальный диммер
│   ├── wb-mr6c.yaml                   # 6-канальное реле
│   ├── wb-msw-v3.yaml                 # Мультисенсор
│   └── wb-mrgbw-d.yaml               # RGB+W контроллер
├── examples/
│   ├── wb-ext-bridge.conf             # Пример конфига
│   └── wb-ext-bridge-scan.conf        # Пример вывода --scan
└── docs/
    ├── adapter-interface.md           # Спецификация адаптерного интерфейса
    └── discovery-engine.md            # Спецификация Discovery Engine
```
