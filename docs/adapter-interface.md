...
1. **Появление устройства.** При загрузке конфига ядро создаёт канонические устройства. Как только все обязательные (required) слоты получают начальные значения из MQTT, вызывается `OnDeviceAdded`.
